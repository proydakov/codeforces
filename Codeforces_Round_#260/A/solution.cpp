...
big_num solve(int k, const std::vector<int>& cnt, std::vector<big_num>& cache)
{
    cache[0] = 0;
    cache[1] = cnt[1];

    for(int i = 2; i <= k; i++) {
        const big_num im1 = cache[i - 1];
        const big_num im2 = cache[i - 2] + big_num(cnt[i]) * big_num(i);
        const big_num score = std::max(im1, im2);
        cache[i] = score;
    }
...
int main()
{
    ...
    std::vector<int> cnt(max_size + 1, 0);
    for(int i = 0; i < n; i++) {
        int temp;
        std::cin >> temp;
        cnt[temp]++;
    ...
    std::vector<big_num> cache(max_size + 1, -1);

    big_num score = solve(max, cnt,  cache);
